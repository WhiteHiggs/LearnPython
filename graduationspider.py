...
#saveinfo用来保存结果到info.txt文件中
    def saveinfo(self,classinfo):
        ...
        for each in classinfo:
            f.writelines('year:' + each['year'] + '\n')
            f.writelines('ID:' + each['ID'] + '\n')
            f.writelines('name:' + each['name'] + '\n')
            f.writelines('college:' + each['college'] + '\n')
            f.writelines('subject:' + each['subject'] +'\n')
            f.writelines('destination:' + each['destination'] + '\n\n')
        f.close()
